...
class Encoder(torch.nn.Module):
    ...
    Encoder class.
    """
    def __init__(self, feature_map_e, channel_number):
        super(Encoder, self).__init__()
        self.feature_map_e = feature_map_e
        self.channel_number = channel_number
        self.main = torch.nn.Sequential(
            torch.nn.Conv2d(channel_number, feature_map_e, 4, 2, 1, bias=False),
            torch.nn.BatchNorm2d(feature_map_e),
            torch.nn.LeakyReLU(0.2, inplace=True),

            torch.nn.Conv2d(feature_map_e, feature_map_e*2, 4, 2, 1, bias=False),
            torch.nn.BatchNorm2d(feature_map_e*2),
            torch.nn.LeakyReLU(0.2, inplace=True),

            torch.nn.Conv2d(feature_map_e*2, feature_map_e*4, 4, 2, 1, bias=False),
            torch.nn.BatchNorm2d(feature_map_e*4),
            torch.nn.LeakyReLU(0.2, inplace=True),

            torch.nn.Conv2d(feature_map_e*4, feature_map_e*8, 4, 2, 1, bias=False),
            torch.nn.BatchNorm2d(feature_map_e*8),
            torch.nn.LeakyReLU(0.2, inplace=True),

            torch.nn.Conv2d(feature_map_e*8, 100, 4, 1, 0, bias=False),
        )
...
def generate_images(dataloader, attack, batch, delta, netG, device):
    output_dir = f"Images/Generated-Images-{attack}-Batch-{batch}-Delta-{delta}"
